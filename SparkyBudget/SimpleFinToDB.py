    ...
    if access_url:
        ...
        try:
            ...
            for account in data.get('accounts', []):
                ...
                balance_insert_data = (
                    account_info['Account ID'], account_info['Account Name'], account_info['Balance Date'],
                    account_info['Balance'], account_info['Available Balance'],
                    organization_domain, organization_name, organization_sf_in_url
                )
                ...
                if 'transactions' in account and account['transactions']:
                    for transaction in account['transactions']:
                        ...
                        transaction_insert_data = (
                            transaction_info['Account ID'], transaction_info['Account Name'], transaction_info.get('Transaction ID', ''),
                            transaction_info['Transaction Posted'], transaction_info.get('Transaction Amount', 0.0),
                            transaction_info.get('Transaction Description', ''), transaction_info.get('Transaction Payee', ''),
