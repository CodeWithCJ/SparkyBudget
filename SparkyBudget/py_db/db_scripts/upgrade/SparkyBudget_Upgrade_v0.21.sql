-- SparkyBudget_Upgrade_v0.21.sql

-- Fix for data loss bug in v0.20 trigger:
-- Restoring TransactionAmountNew to the trigger to prevent manual amount overrides from being lost.
...
BEGIN
    ...
    INSERT OR REPLACE INTO f_transaction (
		...
		(SELECT TransactionAmountNew FROM f_transaction WHERE TransactionID = NEW.TransactionID)
    );
...
-- Update DB_VERSION
UPDATE D_DB SET DB_VERSION = "v0.21";
